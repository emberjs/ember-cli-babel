...
function _shouldIgnoreEmberString(parent, project) {
  ...
  if (_isProjectName(packageName, project)) { return true; }
  if (_isTransitiveDependency(packageName, parent, project)) { return false; }
...
function _getEmberDataPackagesPolyfill(config, parent) {
  ...
  if (typeof parent.name === 'string' && (parent.name === 'ember-data' || parent.name.startsWith('@ember-data/'))) { return; }
...
function _getHelperVersion(project) {
  if (!APP_BABEL_RUNTIME_VERSION.has(project)) {
    let checker = new VersionChecker(project);
    APP_BABEL_RUNTIME_VERSION.set(project, checker.for('@babel/runtime', 'npm').version);
  }
...
function _addDecoratorPlugins(plugins, options, config, parent, project) {
  ...
  if (hasPlugin(plugins, 'babel-plugin-filter-imports')) {
    let checker = new VersionChecker(parent).for('babel-plugin-filter-imports', 'npm');
...
function _shouldIncludeHelpers(options, appInstance) {
  ...
  if (
    ...
    project.ui.writeWarnLine(
      `${_parentName(parent)} attempted to include external babel helpers to make your build size smaller, but your root app's ember-cli-babel version is not high enough. Please update ember-cli-babel to v7.3.0-beta.1 or later.`
    );
  }
