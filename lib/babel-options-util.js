...
function _addImportMetaEnv(plugins, parent, project) {
  const { hasPlugin, addPlugin } = require("ember-cli-babel-plugin-helpers");

  if (hasPlugin(plugins, "@import-meta-env/babel")) {
    if (parent === project) {
      project.ui.writeWarnLine(
        `${_parentName(
          parent
        )} has added the @import-meta-env/babel plugin to its build, but ember-cli-babel provides this by default! You can remove the plugin, or the addon that provided it.`
      );
    }
  } else {
    const isProduction = process.env.EMBER_ENV === "production";
    ...
    addPlugin(
      plugins,
      [
        require.resolve('@import-meta-env/babel'),
        {
          env: {
            DEV: isDebug,
            PROD: isProduction,
