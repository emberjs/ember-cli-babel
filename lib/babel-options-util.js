...
module.exports = {
  ...
  _emberDataVersionRequiresPackagesPolyfill(project) {
  ...
  _getEmberModulesAPIPolyfill(config, parent, project) {
    ...
    if (this._emberVersionRequiresModulesAPIPolyfill()) {
      const ignore = this._getEmberModulesAPIIgnore(parent, project);
  ...
  _getEmberModulesAPIIgnore(parent, project) {
    const ignore = {
      "@ember/debug": ["assert", "deprecate", "warn"],
      "@ember/application/deprecations": ["deprecate"],
    };

    if (this._shouldIgnoreEmberString(parent, project)) {
      ignore["@ember/string"] = [
        "fmt",
        "loc",
        "w",
    ...
    if (this._shouldIgnoreJQuery(parent, project)) {
  ...
  _isProjectName(dependency, project) {
    return project.name && project.name() === dependency;
  },

  _isTransitiveDependency(dependency, parent, project) {
    return (
  ...
  _shouldIgnoreEmberString(parent, project) {
    let packageName = "@ember/string";
    if (this._isProjectName(packageName, project)) {
      return true;
    }
    if (this._isTransitiveDependency(packageName, parent, project)) {
      return false;
    }
  ...
  _shouldIgnoreJQuery(parent, project) {
  ...
  _getEmberDataPackagesPolyfill(config, parent) {
    ...
    if (this._emberDataVersionRequiresPackagesPolyfill(parent)) {
      return [[require.resolve("babel-plugin-ember-data-packages-polyfill")]];
    }
  },
  _getHelpersPlugin(project) {
    return [
      [
        require.resolve("@babel/plugin-transform-runtime"),
        {
          version: this._getHelperVersion(project),
          regenerator: false,
          useESModules: true,
        },
      ],
  ...
  _buildClassFeaturePluginConstraints(constraints, config, parent) {
    ...
    if (!this._shouldHandleTypeScript(config, parent)) {
  ...
  _addDecoratorPlugins(plugins, options, config, parent, project) {
  ...
  _addTypeScriptPlugin(plugins, parent, project) {
    const { hasPlugin, addPlugin } = require("ember-cli-babel-plugin-helpers");
  ...
  _getExtensions(config, parent) {
    let shouldHandleTypeScript = this._shouldHandleTypeScript(config, parent);
  ...
  _shouldHandleTypeScript(config, parent) {
    let emberCLIBabelConfig = config["ember-cli-babel"] || {};
  ...
  _shouldCompileModules(options, project) {
